...
export class KmlGeometrySelectorComponent implements OnInit, OnDestroy {
  ...
  async appendSelectedGeometries(): Promise<void> {
    ...
    try {
      ...
      this.successInsertedCount = Number(result?.insertedCount || (this.mergeGeometry && this.selectedFeatureIds.length >= 2 ? 1 : this.selectedFeatureIds.length));
      this.successTargetTable = `${result?.targetSchema || 'sde'}.${result?.targetTable || this.layerName}`;
