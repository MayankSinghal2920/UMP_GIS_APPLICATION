...
function buildOgrRuntimeEnv(baseEnv = {}) {
  const nextEnv = { ...baseEnv };
  const ogrPath = path.resolve(OGR2OGR_PATH);
  const ogrBin = path.dirname(ogrPath);
  const osgeoRoot = path.basename(ogrBin).toLowerCase() === 'bin'
    ? path.dirname(ogrBin)
    : path.dirname(path.dirname(ogrPath));
  const pathParts = [
    ogrBin,
    path.join(osgeoRoot, 'bin'),
    path.join(osgeoRoot, 'apps', 'gdal', 'bin'),
    path.join(osgeoRoot, 'apps', 'proj', 'bin'),
    path.join(osgeoRoot, 'apps', 'Python312', 'Scripts'),
    nextEnv.PATH || nextEnv.Path || process.env.PATH || '',
  ].filter(Boolean);

  nextEnv.OSGEO4W_ROOT = nextEnv.OSGEO4W_ROOT || osgeoRoot;
  nextEnv.GDAL_DATA = nextEnv.GDAL_DATA || path.join(osgeoRoot, 'apps', 'gdal', 'share', 'gdal');
  nextEnv.GDAL_DRIVER_PATH = nextEnv.GDAL_DRIVER_PATH || path.join(osgeoRoot, 'apps', 'gdal', 'lib', 'gdalplugins');
  nextEnv.PROJ_DATA = nextEnv.PROJ_DATA || path.join(osgeoRoot, 'share', 'proj');
  nextEnv.PROJ_LIB = nextEnv.PROJ_LIB || nextEnv.PROJ_DATA;
  nextEnv.PATH = pathParts.join(path.delimiter);
  nextEnv.Path = nextEnv.PATH;

  return nextEnv;
}

function normalizeOgrError(error) {
  const code = Number(error?.code);
  if (code === 3221225781 || code === -1073741515) {
    return new Error(
      'ogr2ogr failed to start because a required OSGeo4W/GDAL DLL is missing. ' +
      'Open OSGeo4W Shell and run "ogr2ogr --version"; if it fails, reinstall/repair OSGeo4W GDAL. ' +
      'Also verify OGR2OGR_PATH points to the working ogr2ogr.exe.',
    );
  }
...
async function loadKmlIntoTemp(filePath, tempTable, env) {
  ...
  try {
    const result = await execAsync(cmd, {
      env: buildOgrRuntimeEnv({ ...env, PGPASSWORD: DB_PASSWORD }),
      maxBuffer: 50 * 1024 * 1024,
    });
    stderr = result.stderr;
  } catch (error) {
    throw normalizeOgrError(error);
  }
